...
describe('MergeSorter', () => {
    ...
    it('can be instanciated', () => {
        const sorter = new MergeSorter<number>((a, b) => true);
    });

    it('can merge two sorted lists', () => {
        const sorter = new MergeSorter<number>((a, b) => a < b);
    ...
    it('Can sort other values', () => {
        const stringSorter = new MergeSorter<string>();
        expect(stringSorter.sort(['x', 'y', 'a', 'b'])).toEqual(['a', 'b', 'x', 'y']);

        const boolSorter = new MergeSorter<boolean>();
